...
create_image_menu(){
	echo "image name : "
	read iname
	echo "input image size (GB) : "
	read gdi
	image_file_path="$image_path/$iname.img"
	dd if=/dev/zero of="$image_file_path" bs=1024mb count="$gdi"
	echo "create ok !!! save ---- $image_file_path"
}

menu(){
	echo -ne "1.create image\n2.mount iso\nplease select : "
	read sf
	case "$sf" in
	1)
	create_image_menu
